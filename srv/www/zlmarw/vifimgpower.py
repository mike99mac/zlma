...
class Vif_img_set:
  def __init__(self):
    """
    get arguemnts to call the 'vif image set' command 
    """
    print('Content-Type: text/html')       # start the HTML page
    print()
    print('<!DOCTYPE html>')
    print('<html><head><title>Run vif image command</title>')
  ...
  def create_page(self):                   # make the HTML page
    ...
    html_code += f"TODO: gather args to call 'vif image {sub_cmd}' - how much memory/CPU to add/delete\n"
    html_code += "      Args: IMAGE SET image STOrage size|IMAGE SET image CPUs number\n"
